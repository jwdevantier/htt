...
fn select(handles: []*FileHandle, timeout: ?i32) SelectResult {
    ...
    if (is_windows) {
        // TODO: make this, WaitForMultipleObjects
        ...
        const wait_timeout: windows.DWORD = if (timeout) |t| {
            if (t < 0) @panic("invariant broken");
            return @intCast(t);
        } else windows.Infinite;

        const res = windows.WaitForMultipleObjects(@intCast(handles.len), &win_handles, windows.FALSE, // wait for any
            wait_timeout);
        ...
        switch (res) {
            ...
            windows.WAIT_FAILED => {
                ...
                std.debug.print("select, windows, wait error: {}\n", .{err});
                return .{ .ready = 0, .err = error.IoError };
            },
            else => {
                // determine which handle was signalled
                const ndx = res - windows.WAIT_OBJECT_0;
                if (ndx >= 0 and ndx < handles.len) {
                    return .{ .ready = @as(u64, 1) << @intCast(ndx) };
                }
                std.debug.print("select, windows, unhandled signal err\n", .{});
                return .{ .ready = 0, .err = error.IoError };
            },
        }
    } else { //posix land
